...
IscaretInstalled <- require("e1071")
```
...
Data is fist downloaded and processed. Some basic transformations and cleanup are performed, so that `NA` values are omitted. Irrelevant columns such as `user_name`, `raw_timestamp_part_1`, `raw_timestamp_part_2`, `cvtd_timestamp`, `new_window`, and  `num_window` (columns 1 to 7) are removed.
...
## Cross-validation
Cross-validation is performed by splitting the training data in training (75%) and testing (25%) data.
...
## Prediction models
Decision tree and random forest is applied to the data.
...
From the confusion matrices, the Random Forest algorithm performs better than decision trees. The accuracy for the Random Forest model was 0.995 (95% CI: (0.993, 0.997)) compared to 0.739 (95% CI: (0.727, 0.752)) for Decision Tree model. The random Forest model is choosen.
